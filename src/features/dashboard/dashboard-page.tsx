...
export function DashboardPage() {
  ...
  const [isLoadingQuota] = useState(false)
  ...
  useEffect(() => {
    if (quotaStatus) {
      updatePlan(quotaStatus.currentPlan)
    }
  }, [quotaStatus, updatePlan])
  ...
  return (
    <main className="min-h-screen pb-10">
      <div className="mx-auto flex w-full max-w-5xl flex-col gap-6 px-4 pt-4 sm:px-6 lg:px-8">
        <header className="flex flex-col gap-4 rounded-[2rem] border border-border/80 bg-card/70 p-4 shadow-[0_30px_90px_-55px_rgba(0,0,0,0.8)] backdrop-blur sm:flex-row sm:items-center sm:justify-between sm:p-6">
          <div className="flex flex-col gap-2">
            ...
            <p className="max-w-2xl text-sm text-muted-foreground">
              Autenticado como <strong>{session?.username}</strong>. Escribe un
              prompt y revisa la respuesta como en una interfaz de chat real.
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
        ...
        {quotaError ? (
          <Alert tone="warning" title="Estado del plan" message={quotaError} />
        ) : null}
